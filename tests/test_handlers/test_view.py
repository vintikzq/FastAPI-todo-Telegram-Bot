...
@pytest.mark.parametrize(
    ...
    [
        pytest.param([], False, 'empty', False, False,
                     id="empty_list_no_navigation"),
        pytest.param([1], False, "Your tasks list:", True, False,
                     id="single_page_list_no_next_arrow"),
        pytest.param([1], True, "Your tasks list", True, True,
                     id="multi_page_list_shows_next_arrow")
    ]
)
async def test_active_tasks_paginator_should_render_correct_ui_states_based_on_backend_data(
    ...
    callback_data = TaskPaginatorCallBack(
        action=ActionsNav.LIST, page=1, is_archive=False)
...
async def test_task_deletion_should_delete_task_and_show_success_alert(
    ...
    assert callback_kwargs['show_alert'] == False
