...
from src.handlers.task_edit import (
    change_task_deadline,
    change_task_description,
    change_task_name,
    change_task_priority,
    process_new_deadline,
    process_new_task_description,
    process_new_task_name,
    process_new_task_priority,
    process_update_menu,
)
...
async def test_process_update_menu_should_ask_for_update_fields_and_show_keyboard(
    ...
    callback_data = TaskViewCallback(action=ActionsView.UPDATE, page=1, task_id=1)
...
@pytest.mark.asyncio
async def test_change_task_name_should_ask_for_name_and_set_state(callback_query, message, state):
    callback_data = TaskUpdateCallback(to_update=ActionsUpdate.NAME, task_id=1, page=1)
...
async def test_process_new_task_name_should_update_task_name_and_clear_state(
    message, mock_task_service, current_user, bot, prepared_update_state
):
    message.text = "job"

    await process_new_task_name(
        message, mock_task_service, current_user, bot, prepared_update_state
    )
    ...
    assert (await prepared_update_state.get_state()) is None
...
async def test_change_task_description_should_ask_for_description_and_set_state(
    ...
    callback_data = TaskUpdateCallback(to_update=ActionsUpdate.DESCRIPTION, task_id=1, page=1)
...
async def test_process_new_task_description_should_update_description_and_clear_state(
    ...
    await process_new_task_description(
        message, mock_task_service, current_user, bot, prepared_update_state
    )
...
async def test_change_task_priority_should_ask_for_priority_set_keyboard_and_set_state(
    ...
    callback_data = TaskUpdateCallback(to_update=ActionsUpdate.PRIORITY, task_id=1, page=1)
...
async def test_process_new_task_priority_should_update_priority_and_clear_state(
    callback_query, message, mock_task_service, current_user, bot, prepared_update_state
):
...
async def test_change_task_deadline_should_ask_for_deadline_set_keyboard_and_set_state(
    ...
    callback_data = TaskUpdateCallback(to_update=ActionsUpdate.DEADLINE, task_id=1, page=1)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            SimpleCalAct.day,
            UpdateTaskState.waiting_for_new_deadline_date,
            False,
...
async def test_process_new_deadline_should_process_correct_based_on_selected_data(
    ...
    await prepared_update_state.update_data(
        name="Task", priority=TodoPriority.HIGH, description=None
    )
    callback_data = SimpleCalendarCallback(act=act_value)

    with patch(
        "src.handlers.task_edit.SimpleCalendar.process_selection", new_callable=AsyncMock
    ) as mock_selection:
