...
@router.callback_query(
    TaskPaginatorCallBack.filter(
        F.action.in_((ActionsNav.PAGE_DOWN, ActionsNav.PAGE_UP, ActionsNav.LIST))
    )
)
...
async def process_delete_task(
    ...
    if task_id:
        await task_service.delete_task_by_id(current_user.id, task_id=task_id)

        await callback.answer(text="✅ Task successfully deleted", show_alert=False)
...
async def render_tasks_list(
    ...
    kb = (
        get_navigation_buttons(
            tasks=tasks, current_page=current_page, has_next=meta, is_archive=is_archive
        )
        if tasks
        else None
    )
        ...
        try:
            sent_msg = await callback_msg.edit_text(
                ...
                reply_markup=get_task_buttons(
                    task_id=task.id, current_page=page, status=status, is_archive=is_archive
                ),
                parse_mode="HTML",
            )
        except TelegramBadRequest as e:
