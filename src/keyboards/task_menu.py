...
from src.schemas.callbacks import (
    TaskFormCallBack,
    TaskPaginatorCallBack,
    TaskPriorityCallback,
    TaskStatusCallback,
    TaskViewCallback,
)
    ...
    for priority in TodoPriority:
        builder.add(
            InlineKeyboardButton(
                text=priority.label, callback_data=TaskPriorityCallback(value=priority).pack()
            )
        )
    ...
    if current_page > 1:
        buttons.append(
            InlineKeyboardButton(
                text="⬅️",
                callback_data=TaskPaginatorCallBack(
                    action=ActionsNav.PAGE_DOWN, page=current_page - 1, is_archive=is_archive
                ).pack(),
            )
        )

    buttons.append(InlineKeyboardButton(text=f"• {current_page} •", callback_data="ignore"))
    ...
    if has_next:
        buttons.append(
            InlineKeyboardButton(
                text="➡️",
                callback_data=TaskPaginatorCallBack(
                    action=ActionsNav.PAGE_UP, page=current_page + 1, is_archive=is_archive
                ).pack(),
            )
        )
...
def get_task_buttons(
            ...
            status_btns.append(
                InlineKeyboardButton(
                    ...
                    callback_data=TaskStatusCallback(
                        new_status=s, task_id=task_id, page=current_page, is_archive=is_archive
                    ).pack(),
                )
            )
    ...
    delete_btn = InlineKeyboardButton(
        ...
        callback_data=TaskViewCallback(
            action=ActionsView.DELETE, task_id=task_id, page=current_page, is_archive=is_archive
        ).pack(),
    )
    ...
    update_btn = InlineKeyboardButton(
        text="Update ⚙️",
        callback_data=TaskViewCallback(
            action=ActionsView.UPDATE, task_id=task_id, page=current_page, is_archive=is_archive
        ).pack(),
    )
    ...
    if is_archive:
        back_to_list_btn = InlineKeyboardButton(
            ...
            callback_data=TaskPaginatorCallBack(
                action=ActionsNav.ARCHIVE, page=current_page, is_archive=True
            ).pack(),
        )
    else:
        back_to_list_btn = InlineKeyboardButton(
            text="🔙 Back to list",
            callback_data=TaskPaginatorCallBack(action=ActionsNav.LIST, page=current_page).pack(),
        )
